...
void flush_to_db_task(void *pvParameters)
{
    ...
    memcpy(copied_read_watts, read_watts, (600*sizeof(int)));
    xSemaphoreGive(batton);
    // wait for 8 minutes, for the sms to be sent, this is because only one file can be opened at a time for operation.
    xSemaphoreTake( sms_wait, pdMS_TO_TICKS(480000));
    
    check_and_update_month_file();

    int16_t peak_watt = 0;
    int16_t least_watt = 0;
    int16_t sum = 0;
    for(;;)
    {
        for( int i=0; i<(sizeof(copied_read_watts)/sizeof(int16_t)); i++)
        {
            ...
            if( copied_read_watts[i] < least_watt )
...
void check_for_calls_task( void *pvParameters )//this will always be running to listen, I don't know if interrupts can work though, I don't even know how it works.
{
    for(;;)
    {
        if( recieved_call(&recipient_no) )
            xTaskCreatePinnedToCore(send_data_task, "send sms message", 10000, NULL, 2, &send_sms, 0);
    }
}
...
void check_and_update_month_file()
{
    ...
    {
        filename.clear();
        filename.append(mon < 10 ? ("0" + std::to_string(mon)) : std::to_string(mon), 0, 2);
        filename += "-";
...
void setup()
{
    ...
    xTaskCreatePinnedToCore(read_vi_task, "read real power from ADC", 10000, NULL, 3, &read_vi, 0);
    xTaskCreatePinnedToCore(check_for_calls_task, "check for incoming calls", 1000, NULL, 1, &check_for_calls, 0);
    // xTaskCreatePinnedToCore(test_1, "testing 1", 5000, NULL, 3, &task1, 0);
    delay(500);
}
...
//         {
//             xSemaphoreTake( batton, portMAX_DELAY);
//             xTaskCreatePinnedToCore(test_2, "testing 2", 10000, NULL, 1, &task2, 0);
//             xSemaphoreGive(batton);
//             counter = 0;
//         }
...
// void test_2(void *pvParameters)
// {
//     xSemaphoreTake( batton, portMAX_DELAY );
//     int copy_array[10];
//     memcpy(copy_array, counter_array, 10*sizeof(int));
//     xSemaphoreGive( batton );
//     for(;;)
//     {
//         for(int i=0; i<(sizeof(copy_array)/sizeof(int)); i++){
